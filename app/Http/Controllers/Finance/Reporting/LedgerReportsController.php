...
class LedgerReportsController extends BaseController
{
    ...
    public function ledgerPeriodWisePDF(Request $request)
    {
        ...
        $ledgers = $voucherModel->getLedgerPeriodWise($accountHeadVO);
...
//        echo ($totalbalance);
        if(empty($ledgers)){
            \Session::flash('ledger', "No General Ledger is found!");
            return redirect()->back();
        }
        $pdf = PDF::loadview('Finance.Reporting.GeneralLedgerReports.generalLedgerReport',
            compact('ledgers','startdate','enddate','totalbalance','debitbalance','creditbalance'));
        }
    }
        //echo ($totalbalance);
      /*$pdf = PDF::loadview('Finance.Reporting.GeneralLedgerReports.generalLedgerReport',
